 ...
 * @author  akio sai <actionfist@gmail.com>
...
{
    public static $PM_Url='https://pm.ineedadeveloper.com/';

    //https://ineedadeveloper.com/test_an
    public function test()
    {
        $gw = new Nmi();
        $gw->setLogin(env("NMI_Key"));

//        $gw->setBilling("Akio", "Xai", "Acme, Inc.", "123 Main St", "Suite 200", "Beverly Hills",
//            "CA", "90210", "US", "555-555-5555", "555-555-5556", "actionfist@gmail.com",
//            "www.example.com");

        $gw->billing['firstname'] = "Akio";
        $gw->billing['lastname'] = "Xai";

//        $gw->setShipping("Akio1", "Xai1", "na", "124 Shipping Main St", "Suite Ship", "Beverly Hills",
//            "CA", "90210", "US", "actionfist@gmail.com");

        $gw->setOrder("1238", "Big Order", 1, 1, "PO1234", "65.192.14.10");

        $r = $gw->doSale("12.00", "4111111111111111", "1010");
        print_r ($gw->responses);
    }

    //https://ineedadeveloper.com/go/pm
    public function goPm()
    {
        $url=self::$PM_Url.'signin/fromMain/'.$this->base64_url_encode(Auth::user()->email);
        return Redirect::away($url);
//        $user = User::find(2);
//        echo $this->CreatePMUser($user);
    }

    public function CreatePMUser($user){
        $client = new GuzzleHttp\Client();
        $url = PublicController::$PM_Url.'signup/create_account_from_MainServer';

        $myBody['first_name'] = $user->first_name;
        $myBody['last_name'] = $user->last_name;
        $myBody['email'] = $user->email;
        $myBody['job_title'] = $user->slug;

        $request = $client->post($url,  ['json'=>$myBody]);
        ...
        if (Session::has('user_id')) {
            ...
            if (!empty($request['code'])) {
                if ($request['code'] === $user->verification_code) {
                    ...
                    $user->save();
                    $json['type'] = 'success';
                    //send PM server to create Data
                    $this->CreatePMUser($user);
                    //send mail
    ...
    public function emailTest(){
        $admin_template = DB::table('email_types')->select('id')->where('email_type', 'admin_email_registration')->get()->first();
        $template_data = EmailTemplate::getEmailTemplateByID($admin_template->id);
        $email_params['name'] = Helper::getUserName(29);
        $email_params['email'] = 'amgmobiledev@gmail.com';
        $email_params['link'] = url('profile/an-mg');
        $res= Mail::to(env('MAIL_FROM_ADDRESS'))
            ->send(
                new AdminEmailMailable(
                    'admin_email_registration',
                    $template_data,
