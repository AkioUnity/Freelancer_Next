<template>
    <div v-if="notify">
        <div v-for="(user, index) in users" :key="index" @click="startChat(user.id)" class="wt-ad" :class="[active_id===user.id ? 'wt-active' : '' , user.status_class]">
            ...
            <figure @click="startVideoChat(user.id)">
                <img :src="image_path+'/images/custom/video-call-btn.png'" alt="video-call btn">
            </figure>
        </div>
    </div>
    <div v-else>
...
import Event from '../event.js';
    export default{
        ...
        methods: {
            ...
            startVideoChat(id){
                let room=(id>Laravel.user.id)?id+'_'+Laravel.user.id:Laravel.user.id+'_'+id;
                //room=btoa(room);
                let queryString="isAdmin=1&room="+room+"&k="+Laravel.user.name;
                queryString=btoa(queryString);
                window.open("https://pm.ineedadeveloper.com/chat/pages/r.html?"+queryString, "_blank");
            }
        },
        mounted: function () {
