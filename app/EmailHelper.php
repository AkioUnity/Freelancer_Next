...
class EmailHelper extends Model
{
    ...
    public static function getEmailFooter()
    {
        ...
        $copyright = !empty($setting) && !empty($setting['copyright']) ? $setting['copyright'] : 'Copyright All Rights Reserved';
        ?>
        </div>
        </div>
        </div>
    ...
    public static function getSiteTitle()
    {
        ...
        $title = !empty($settings) && !empty($settings[0]['title']) ? $settings[0]['title'] : 'MyCompany';
        return $title;
    }
    ...
    {
        $setting = SiteManagement::getMetaValue('email_data');
        $email = !empty($setting) && !empty($setting[0]['from_email']) ? $setting[0]['from_email'] : 'info@amgly.com';
        return $email;
    }
    ...
    {
        $setting = SiteManagement::getMetaValue('email_data');
        $email_id = !empty($setting) && !empty($setting[0]['from_email_id']) ? $setting[0]['from_email_id'] : 'info@amgly.com';
        return $email_id;
    }
    ...
    public static function getSignature()
    {
        ob_start();
        $setting = SiteManagement::getMetaValue('email_data');
        $sender_name = !empty($setting) && !empty($setting[0]['sender_name']) ? $setting[0]['sender_name'] : 'MyCompany';
        $sender_tagline = !empty($setting) && !empty($setting[0]['sender_tagline']) ? $setting[0]['sender_tagline'] : 'A Better Workplace for Employers and Freelancers';
