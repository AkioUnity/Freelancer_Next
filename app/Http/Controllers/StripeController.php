...
class StripeController extends Controller
{
    ...
    public function postPaymentWithNmi(Request $request)
    {
        $settings = SiteManagement::getMetaValue('commision');
        $currency = !empty($settings[0]['currency']) ? $settings[0]['currency'] : 'USD';
        $product_id = Session::has('product_id') ? session()->get('product_id') : '';
        ...
        $order_id= 'NMI_' . $unique_code . '_' . $product_id;
        try {
            $gw = new Nmi();
            $gw->setLogin(env("NMI_Key"));
            $gw->billing['firstname'] = $request->firstname;
            $gw->billing['lastname'] = $request->lastname;
            $gw->setOrder($order_id, $product_title, 1, 1, "PO1234", "65.192.14.10");
            $ccexp=$request->ccexpmm.$request->ccexpyy; //"1010"  //999
            //            $r = $gw->doSale("12.00", "4111111111111111", $ccexp,$request->cvv);
            $r = $gw->doSale($product_price, $request->ccnumber, $ccexp,$request->cvv);
            $response = $gw->responses;

            if ($response['response']==1) {
                $fee = !empty($response['application_fee_amount']) ? $response['application_fee_amount'] : 0;
                $invoice = new Invoice();
                $invoice->title = 'Invoice';
                $invoice->price = $product_price;
                $invoice->payer_name = filter_var($request->firstname.' '.$request->lastname, FILTER_SANITIZE_STRING);
                $invoice->payer_email = filter_var(Auth::user()->email, FILTER_SANITIZE_EMAIL);
                $invoice->seller_email = 'actionfist@gmail.com';
                $invoice->currency_code = filter_var($currency, FILTER_SANITIZE_STRING);
                $invoice->payer_status = '';
                $invoice->transaction_id = filter_var($response['transactionid'], FILTER_SANITIZE_STRING);
                $invoice->invoice_id = filter_var($order_id, FILTER_SANITIZE_STRING);
                $invoice->customer_id = filter_var($user_id, FILTER_SANITIZE_STRING);
                $invoice->shipping_amount = 0;
                $invoice->handling_amount = 0;
                $invoice->insurance_amount = 0;
                $invoice->sales_tax = 0;
                $invoice->payment_mode = filter_var('credit card', FILTER_SANITIZE_STRING);
                $invoice->paypal_fee = $fee;
                $invoice->paid = 1;
                $product_type = $type;
                $invoice->type = $product_type;
...
//        session()->put(['code' => $json['type'], 'message' => $json['message']]);
