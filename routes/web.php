...
Auth::routes();
// Cache clear route  https://ineedadeveloper.com/cache-clear
Route::get(
    'cache-clear',
    function () {
...
// Cache clear route  https://ineedadeveloper.com/view-clear
Route::get(
    'view-clear',
    function () {
...
//send email test
Route::get('test/email', 'PublicController@emailTest');
//test request
Route::get('test_an', 'PublicController@test')->name('test_an');
...
Route::group(
    ...
    function () {
        Route::get('/exam', function(){
            ...
            return view('exam_home', compact('topics', 'questions'));
        });
...
//            $answers = Answer::where('topic_id','=',$topic->topic_id)->first();
        ...
        Route::get('start_quiz/{id}/finish', function($id){
            ...
            if ($count_questions>$topic->number_of_questions && $topic->number_of_questions>0)
                $count_questions=$topic->number_of_questions;
            $ans= Answer::all();
            $q= Question::all();
...
Route::group(
    ...
    function () {
        Route::get('go/pm', 'PublicController@goPm')->name('goPm');
...
Route::post('addmoney/postPaymentWithNmi', 'StripeController@postPaymentWithNmi');
